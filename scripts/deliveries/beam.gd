...
func _ready() -> void:
	if has_node("ColorRect"):
		var cr := $ColorRect as ColorRect
		cr.size = Vector2(40.0, 1920.0)
		cr.position = Vector2(-20.0, -1920.0)
		cr.color = Color(0.4, 0.9, 1.0, 0.7)
...
func _execute_hit() -> void:
	...
	for enemy_node in enemies:
		...
		if not is_instance_valid(enemy):
			continue
		if abs(enemy.global_position.x - global_position.x) <= beam_width / 2.0:
...
func _process(delta: float) -> void:
	...
	if has_node("ColorRect"):
		var color_rect: ColorRect = get_node("ColorRect") as ColorRect
		if color_rect != null:
			color_rect.modulate.a = 1.0 - (_elapsed / lifetime)
